...
@implementation STFileRepository
...
-(NSArray *) getAll {
  NSString *request = [NSString stringWithFormat:@"SELECT %@ FROM %@",
                       [[STFileRepository getSelectionColumnsString]componentsJoinedByString:@","],
  ...
  [queue inDatabase:^(FMDatabase * _Nonnull db) {
    ...
    while ([resultSet next]) {
      FileDbo * dbo = [STFileRepository getFileDboFromResultSet:resultSet];
      if(dbo) {
...
-(FileDbo *) getByFileId:(NSString *) fileId {
  ...
  [queue inDatabase:^(FMDatabase * _Nonnull db) {
    ...
    if([resultSet next]) {
      dbo = [STFileRepository getFileDboFromResultSet:resultSet];
    }
    [resultSet close];
  }];
  [queue close];
