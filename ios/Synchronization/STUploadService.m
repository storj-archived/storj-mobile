...
//  StorjMobile
//
//  Created by Yaroslav Vorobiov Mac on 01.06.2018.
...
static STUploadService *instance = nil;
...
@implementation STUploadService
{
  @private
...
+(instancetype) sharedInstance
{
  dispatch_once(&onceToken, ^{
    instance = [[STUploadService alloc] init];
                     ...
                     localPath: (NSString *) localPath
{
  ...
  STFileUploadCallback *uploadCallback = [STUploadService getUploadCallbackWithWaiter : semaphore];
                      ...
                      localPath: (NSString *) localPath
{
  ...
  dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
  STFileUploadCallback *uploadCallback = [STUploadService getSyncCallbackWithWaiter:(dispatch_semaphore_t) semaphore syncEntryId: syncEntryId];
...
{
  __block STFileUploadCallback *uploadNotifyCallback = [STUploadService getUploadNotifyCallback];
                                        ...
                                        syncEntryId: (int) syncSyncEntryId
{
  STFileUploadCallback *uploadCallback = [[STFileUploadCallback alloc] init];
  __block STFileUploadCallback *uploadNotifyCallback = [STUploadService getUploadNotifyCallback];
  __block STSyncQueueEntryModel *syncModel = nil;
  ...
  void (^updateDbo)(UpdateDboBlock) = ^(UpdateDboBlock callback)
  {
    ...
    if([[syncRepo updateWithModel: [[STSyncQueueEntryModel alloc] initWithDbo:(SyncQueueEntryDbo *) dbo]] isSuccess])
    {
...
+(STFileUploadCallback *) getUploadNotifyCallback
{
  ...
  uploadCallback._uploadCompleteBlock = ^(long fileHandle, NSString *fileId) {
    ...
    [[STStorjBackgroundServices sharedInstance] sendEventWithName:STEventNames.EVENT_FILE_UPLOAD_SUCCESSFULLY
                                                           body:bodyDict];
  };
  ...
  uploadCallback._uploadProgressBlock = ^(long fileHandle, double uploadProgress, double uploadedBytes) {
    ...
    [[STStorjBackgroundServices sharedInstance]  sendEventWithName:STEventNames.EVENT_FILE_UPLOAD_PROGRESS
                                                            body: body];
  };
  
  uploadCallback._uploadErrorBlock = ^(long fileHandle, int errorCode, NSString *errorMessage) {
    [Logger log:[NSString stringWithFormat:@"onError: %d, %@", errorCode, errorMessage]];
    [[STStorjBackgroundServices sharedInstance]  sendEventWithName:STEventNames.EVENT_FILE_UPLOAD_ERROR
  ...
  uploadCallback._uploadStartBlock = ^(long fileHandle) {
    ...
    [[STStorjBackgroundServices sharedInstance]  sendEventWithName:STEventNames.EVENT_FILE_UPLOAD_START
                                                            body:eventDict];
  };
